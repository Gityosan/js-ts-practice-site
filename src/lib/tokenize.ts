...
export function matchBrackets(tokens: Token[]): Map<number, number> {
  ...
  for (const t of tokens) {
    ...
    if (open.includes(t.value)) {
      stack.push({ ch: t.value, idx: t.index });
    } else {
      const top = stack.pop();
      if (top && open.indexOf(top.ch) === close.indexOf(t.value)) {
 ...
 */
export function explainToken(tokens: Token[], idx: number): TokenExplain {
  const t = tokens[idx];
  switch (t.type) {
    case "keyword": {
      return {
